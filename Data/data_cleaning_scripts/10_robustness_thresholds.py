...
panel_base = pd.read_csv('Data/Panel/panel_with_income.csv')
...
def build_treatment(aid, threshold_m):
    """Build treat/post variables for a given threshold in USD millions."""
    aid_sorted = aid.sort_values(['Country Code','year'])
    aid_sorted['cumul_usd'] = aid_sorted.groupby('Country Code')['usd_const_2021_sum'].cumsum()
...
def run_did(df, outcome, label):
    ...
    p  = mod.pvalues['did']
    n  = int(mod.nobs)
    nc = df['Country Code'].nunique()
    sig = '***' if p<0.01 else '**' if p<0.05 else '*' if p<0.1 else ''
    return c, se, t, p, n, nc, sig
...
for ocol, olbl in outcomes:
    ...
    print(f"{'#'*70}")
    print(f"\n  {'Threshold':>12} {'N Treated':>10} {'N Obs':>8} {'Coef':>10} {'SE':>8} {'t':>7} {'p':>7} {'':>4}")
    print(f"  {'-'*70}")

    for thresh in thresholds:
        try:
            # Build treatment for this threshold
            crossed = build_treatment(aid, thresh)
            
            # Merge into panel
            panel = panel_base.copy()
            # Drop existing treatment cols and rebuild
            panel = panel.drop(columns=['treat_500m','post_500m','treat_year'], errors='ignore')
            panel = panel.merge(crossed[['Country Code','treat_year']], 
                               on='Country Code', how='left')
            panel['treat_500m'] = panel['treat_year'].notna().astype(int)
            panel['post_500m']  = (
                panel['treat_500m']==1) & (panel['year'] >= panel['treat_year']
            ).astype(int)

            n_treated = panel[panel['treat_500m']==1]['Country Code'].nunique()

            c, se, t, p, n, nc, sig = run_did(panel, ocol, olbl)
            marker = ' ← MAIN SPEC' if thresh==500 else ''
            print(f"  ${thresh:>9}M {n_treated:>10} {n:>8} {c:>+10.3f} {se:>8.3f} {t:>7.2f} {p:>7.3f} {sig:>4}{marker}")

        except Exception as e:
            print(f"  ${thresh:>9}M  ERROR: {e}")

# ── Also run SSA robustness ──
print(f"\n\n{'#'*70}")
...
for ocol, olbl in outcomes:
    print(f"\n  {olbl}")
    print(f"  {'Threshold':>12} {'N Treated':>10} {'N Obs':>8} {'Coef':>10} {'SE':>8} {'t':>7} {'p':>7} {'':>4}")
    print(f"  {'-'*65}")
    ...
    for thresh in thresholds:
        try:
            ...
            c, se, t, p, n, nc, sig = run_did(ssa, ocol, olbl)
            marker = ' ← MAIN SPEC' if thresh==500 else ''
            print(f"  ${thresh:>9}M {n_treated:>10} {n:>8} {c:>+10.3f} {se:>8.3f} {t:>7.2f} {p:>7.3f} {sig:>4}{marker}")
